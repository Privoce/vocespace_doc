...
<Tabs>
  <Tab label="Linux/macOS">
    ...
    show_help() {
        ...
        echo ""
        echo "Options:"
        echo " -r, --resolution RESOLUTION Set resolution (e.g.: 4k, 1080p, 720p)"
    ...
    start_vocespace_container() {
        ...
        # Add resolution environment variable
        if [[ -n "$RESOLUTION" ]]; then
            ENV_ARGS+=("-e" "NEXT_PUBLIC_RESOLUTION=$RESOLUTION")
            echo "Setting resolution: $RESOLUTION"
        fi
  ...
  <Tab label="Windows">
    ...
    REM Applicable to Windows systems
    ...
    REM If resolution is set, add to environment variables
    if not "%RESOLUTION%"=="" (
        echo Setting resolution: %RESOLUTION%
        if "!ENV_VARS!"=="" (
    ...
    REM Show environment variables to be used
    if not "!ENV_VARS!"=="" (
    ...
    echo 4. Run the following command in the terminal: livekit-server --dev --bind 0.0.0.0
    echo.
    ...
    if %errorlevel% equ 0 (
        ...
        echo ⚠️  LiveKit Server is not found, please install it manually as per the instructions above
    )
